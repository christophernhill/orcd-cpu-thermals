...
* Banner ("recording CSV to ...") and Ctrl-C summary go to ``stderr`` so
  they never end up inside the CSV file.
* The file is opened in append mode and is line-buffered, so ``tail -f``
  on the file shows new readings as they arrive.
* The header row is only written when the destination file is new or
  empty, which makes re-using a path concatenate cleanly.
"""
...
class CsvRenderer:
    """Stream temperature readings to a CSV file."""
    ...
    def __init__(self, path: str) -> None:
        ...
        self._rows = 0
        self._stream = None  # opened lazily in start()
        self._writer = None
    ...
    def start(self, labels: Sequence[str]) -> None:
        ...
        if needs_header:
            ...
            self._stream.flush()
        sys.stderr.write(f"[cpu_thermals] recording CSV to {self._path}\n")
    ...
    def row(self, readings: Sequence[Reading]) -> None:
        ...
        for r in readings:
            self._writer.writerow((ts, self._node, r.label, f"{r.celsius:.1f}"))
            self._rows += 1
        self._stream.flush()
    ...
    def stop(self) -> None:
        if self._stream is not None:
            try:
                ...
                pass
        sys.stderr.write(
            f"[cpu_thermals] wrote {self._rows} rows to {self._path}\n"
        )
